...
//               <progress // to be worked on tommorow
//                 id="progressBar"
...
//   const [showModal, setShowModal] = useState(false);
//   const [showCongratulatoryMessage, setShowCongratulatoryMessage] = useState("");
//   const [requiredCurrency, setRequiredCurrency] = useState(0);
//   const [feedButtonState, setFeedButtonState] = useState(false);

//   useEffect(() => {
//     if (petData) {
//         console.log(petData)
//       // Calculate the required currency based on the current progress bar value
//       const currency = Math.pow(2, Math.floor(petData.pet_progress_bar / 10)) * 5;
//       setRequiredCurrency(currency);
//     }
//   }, [petData]);
...
//         }, 3000); // Message will disappear after 3 seconds
//       } else {
//         updatedPet.pet_currency -= requiredCurrency; // Deduct the required currency
//         updatedPet.pet_progress_bar = Math.min(updatedPet.pet_progress_bar + 10, 100); // Add 10 progress, but max it out at 100
//       }
...
//           <div className="flex flex-col items-center">
//             <img
//             // hardcoded ang character asset. fix this later.
//               src={`../../../src/assets/${petData.pet_type}.gif`} // Example image
//               alt="Pet"
//               className="w-10 h-64 md:w-96 md:h-96 object-contain transition-all duration-500"
//             />
//             <div className="w-64 md:w-96">
...
const Pets: React.FC<PetsProps> = ({ petData, onPetAdded, onPetUpdated }) => {
  ...
  useEffect(() => {
    if (petData) {
      console.log(petData);
      // Initialize the required currency when petData changes
  ...
  const handleFeedPet = () => {
    if (petData && petData.pet_currency >= requiredCurrency) {
      ...
      // Increment the required currency for the next feeding
